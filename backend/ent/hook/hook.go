...
// The DesktopModelCredentialFunc type is an adapter to allow the use of ordinary
// function as DesktopModelCredential mutator.
type DesktopModelCredentialFunc func(context.Context, *ent.DesktopModelCredentialMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DesktopModelCredentialFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DesktopModelCredentialMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DesktopModelCredentialMutation", m)
}
