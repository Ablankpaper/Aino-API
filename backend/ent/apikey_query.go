...
type APIKeyQuery struct {
	...
	withDesktopModelCredentials *DesktopModelCredentialQuery
...
// QueryDesktopModelCredentials chains the current query on the "desktop_model_credentials" edge.
func (_q *APIKeyQuery) QueryDesktopModelCredentials() *DesktopModelCredentialQuery {
	query := (&DesktopModelCredentialClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withDesktopModelCredentials: _q.withDesktopModelCredentials.Clone(),
...
// WithDesktopModelCredentials tells the query-builder to eager-load the nodes that are connected to
// the "desktop_model_credentials" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithDesktopModelCredentials(opts ...func(*DesktopModelCredentialQuery)) *APIKeyQuery {
	query := (&DesktopModelCredentialClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDesktopModelCredentials = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			_q.withDesktopModelCredentials != nil,
		}
	)
	...
	if query := _q.withDesktopModelCredentials; query != nil {
		if err := _q.loadDesktopModelCredentials(ctx, query, nodes,
			func(n *APIKey) { n.Edges.DesktopModelCredentials = []*DesktopModelCredential{} },
			func(n *APIKey, e *DesktopModelCredential) {
				n.Edges.DesktopModelCredentials = append(n.Edges.DesktopModelCredentials, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadDesktopModelCredentials(ctx context.Context, query *DesktopModelCredentialQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *DesktopModelCredential)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(desktopmodelcredential.FieldAPIKeyID)
	}
	query.Where(predicate.DesktopModelCredential(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.DesktopModelCredentialsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
