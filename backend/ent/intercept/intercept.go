...
// The DesktopModelCredentialFunc type is an adapter to allow the use of ordinary function as a Querier.
type DesktopModelCredentialFunc func(context.Context, *ent.DesktopModelCredentialQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f DesktopModelCredentialFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.DesktopModelCredentialQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.DesktopModelCredentialQuery", q)
}

// The TraverseDesktopModelCredential type is an adapter to allow the use of ordinary function as Traverser.
type TraverseDesktopModelCredential func(context.Context, *ent.DesktopModelCredentialQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseDesktopModelCredential) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseDesktopModelCredential) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.DesktopModelCredentialQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.DesktopModelCredentialQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.DesktopModelCredentialQuery:
		return &query[*ent.DesktopModelCredentialQuery, predicate.DesktopModelCredential, desktopmodelcredential.OrderOption]{typ: ent.TypeDesktopModelCredential, tq: q}, nil
