...
func (_q *UserQuery) QueryDesktopModelCredentials() *DesktopModelCredentialQuery {
...
func (_q *UserQuery) WithDesktopModelCredentials(opts ...func(*DesktopModelCredentialQuery)) *UserQuery {
	query := (&DesktopModelCredentialClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withDesktopModelCredentials; query != nil {
		if err := _q.loadDesktopModelCredentials(ctx, query, nodes,
			func(n *User) { n.Edges.DesktopModelCredentials = []*DesktopModelCredential{} },
			func(n *User, e *DesktopModelCredential) {
...
func (_q *UserQuery) loadDesktopModelCredentials(ctx context.Context, query *DesktopModelCredentialQuery, nodes []*User, init func(*User), assign func(*User, *DesktopModelCredential)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(desktopmodelcredential.FieldUserID)
	}
	query.Where(predicate.DesktopModelCredential(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.DesktopModelCredentialsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
