...
func (_q *GroupQuery) QueryDesktopModelCredentials() *DesktopModelCredentialQuery {
...
func (_q *GroupQuery) WithDesktopModelCredentials(opts ...func(*DesktopModelCredentialQuery)) *GroupQuery {
	query := (&DesktopModelCredentialClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withDesktopModelCredentials; query != nil {
		if err := _q.loadDesktopModelCredentials(ctx, query, nodes,
			func(n *Group) { n.Edges.DesktopModelCredentials = []*DesktopModelCredential{} },
			func(n *Group, e *DesktopModelCredential) {
...
func (_q *GroupQuery) loadDesktopModelCredentials(ctx context.Context, query *DesktopModelCredentialQuery, nodes []*Group, init func(*Group), assign func(*Group, *DesktopModelCredential)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(desktopmodelcredential.FieldGroupID)
	}
	query.Where(predicate.DesktopModelCredential(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.DesktopModelCredentialsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
